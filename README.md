...
| **[Ollama](https://ollama.com)** | Runs the local LLM (`qwen3:8b`) that powers SQL generation and typo correction — no cloud API calls |
| **[pandas](https://pandas.pydata.org)** | Reads Excel files and shapes query results into DataFrames for display |
| **[sqlglot](https://github.com/tobymao/sqlglot)** | Parses and validates every generated SQL query before it's allowed to execute |
| **[Typer](https://typer.tiangolo.com)** | Builds the command-line interface |
...
- **Usable from anywhere.** Installed once, callable as a plain command (`nl2sql "..."`) from any terminal, on any folder, without needing to `cd` into the project directory or type `python cli.py`.
...
Your question
     │
     ▼
1. Load the file into DuckDB (CSV or Excel)
     │
     ▼
2. Extract schema: column names, types, sample values
     │
     ▼
3. Correct typos in your question (using an LLM, aware of your real column/value names)
     │
     ▼
4. Build a prompt: schema + rules + your (corrected) question
     │
     ▼
5. Ask the local LLM (qwen3:8b via Ollama) to generate SQL
     │
     ▼
6. Validate the SQL: only SELECT statements, only real columns/aliases
     │
     ├─ Invalid? → retry once with the error fed back to the LLM
     │
     ▼
7. Execute the validated SQL against DuckDB
     │
     ▼
8. Show you the SQL used and the result table
```

The key design decision behind this pipeline is **step 2 and step 6**: the LLM is never left to guess at your data's shape, and its output is never trusted blindly. It's shown the real schema before generating anything, and every query is checked against that same schema before it's allowed to run. This is what keeps the tool from hallucinating columns that don't exist or executing something destructive.
...
```bash
python cli.py <file> "<question>"
```
It loads the file, extracts the schema, corrects the question, builds the prompt, generates SQL, checks for `CANNOT_ANSWER`, validates the SQL (retrying once if invalid), executes it, and prints both the generated SQL (syntax-highlighted via Rich) and the result table.
...
```bash
python cli.py <path-to-csv-or-xlsx> "<your question>"
```

Examples:
```bash
python cli.py examples/sample_sales.csv "show me total sales by category"
python cli.py examples/sample_sales.csv "electronics orders in the north or south region"
python cli.py examples/sample_sales.csv "what's the best selling product"
```

## Example Walkthrough

Given `examples/sample_sales.csv` (order_id, product, category, price, quantity, order_date, region):

```bash
$ python cli.py examples/sample_sales.csv "shwo me toatl sales by categry"
...
This is a fully working end-to-end pipeline for single-file queries run from within the project directory.

---

## Roadmap — What's Left to Build

- [ ] **Global CLI install** — package the tool properly (`pip install -e .` with an entry point) so it's callable as `nl2sql "..."` from any terminal, in any directory, without needing to `cd` into the project or type `python cli.py`
...
- [ ] **Auto-discovery of data files** — when run in a folder, automatically detect CSV/Excel files present, and if there's exactly one, use it without asking; if there's more than one, prompt the user to pick which one to query
...
- [ ] **README polish** — usage GIF/screenshot at the top once the CLI is globally installable, since that's the most convincing thing a first-time visitor to the repo will see
